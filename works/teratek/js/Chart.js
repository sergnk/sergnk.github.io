...
(function(){
		...
		showTooltip : function(ChartElements, forceRedraw){
			...
			if (ChartElements.length > 0){
				...
				if (this.datasets && this.datasets.length > 1) {
					...
					var indexOfAnyElement = function (dataArray, elements) {
						for (var i = 0; i < dataArray.length; i++) {
							for (var j = 0; j < elements.length; j++) {
								if (dataArray[i] == elements[j]) {
									return i;
								}
							}
						}
					...
					for (var i = this.datasets.length - 1; i >= 0; i--) {
						if (this.datasets[i].withoutTooltip) { continue; }
						dataArray = this.datasets[i].points || this.datasets[i].bars || this.datasets[i].segments;
						dataIndex = indexOfAnyElement(dataArray, ChartElements);
						if (dataIndex !== -1){
							break;
						}
					}
					var tooltipLabels = [],
						...
						medianPosition = (function(index) {
							...
							helpers.each(this.datasets, function(dataset){
								if (dataset.withoutTooltip) { return; }
					...
					new Chart.MultiTooltip({
						...
						tooltipTitleXPadding: this.options.tooltipTitleXPadding,
						cornerRadius: this.options.tooltipCornerRadius,
						labels: tooltipLabels,
						legendColors: tooltipColors,
						legendColorBackground : this.options.multiTooltipKeyBackground,
						title: ChartElements[0].label.label,
						chart: this.chart,
						ctx: this.chart.ctx,
	...
	Chart.Tooltip = Chart.Element.extend({
		draw : function(){
			...
			var texts = JSON.parse(this.text);

			var maxWidthFunc = function (result, text) {
			...
			var heightSumFunc = function (sum, text) {
				return sum + text.fontSize + text.paddingTop;
			};

			var tooltipWidth = texts.reduce(maxWidthFunc, 0) + 2*this.xPadding,
				tooltipRectHeight = texts.reduce(heightSumFunc, 0) + 2*this.yPadding,
			...
			if(this.custom){
				...
				for (var i = 0; i < texts.length; i++) {
					var fontSize = texts[i].fontSize || this.fontSize;
					ctx.font = fontString(fontSize,this.fontStyle,this.fontFamily);
					ctx.fillText(texts[i].val, tooltipX + tooltipWidth/2, tooltipY + (fontSize + texts[i].paddingTop) * (i + 1));
				}
			}
		}
	});
	...
	Chart.MultiTooltip = Chart.Element.extend({
		...
		draw : function(){
			...
			if(this.custom){
				...
				ctx.fillText(this.title,this.x + this.width/2, this.getLineHeight(0));
		...
		initialize:  function(data){
			...
			helpers.each(data.datasets,function(dataset){
				...
				var datasetObject = {
					...
					withoutTooltip: dataset.withoutTooltip,
					points : [],
				...
				this.eachPoints(function(point, index){
					helpers.extend(point, {
						x: this.scale.calculateX(point.index),
						y: this.scale.endPoint
					});
					point.save();
				}, this);
		...
		draw : function(ease){
			...
			helpers.each(this.datasets,function(dataset){
				...
				helpers.each(dataset.points,function(point){
					...
					if (index>0){
						if (dataset.dashStyle && dataset.dashStyle instanceof Array){
							helpers.drawDashedLine(ctx, dataset.points[prevIndex].x, dataset.points[prevIndex].y, point.x, point.y, dataset.dashStyle);
						}
						else if(this.options.bezierCurve){
							ctx.bezierCurveTo(
								dataset.points[prevIndex].controlPoints.outer.x,
								dataset.points[prevIndex].controlPoints.outer.y,
