...
$url = 'https://feeds.blogcertified.com/posts?categories=1122&num_of_posts=5';
...
if ( ! empty( $articles ) ) {
  ...
  if ( isset( $articles->next_page ) ) {
    $pagePrev = ($articles->next_page - 2) >= 1 ? ($articles->next_page - 2) : 0;
    $nextPage = $articles->next_page;
  }

	$articles = $articles->data;

	if ( ! empty( $articles ) ) {
		foreach ( $articles as $ar_k => $ar_v ) {
			$articles_arr[ $ar_k ] = getPost( $ar_v );
			$articles_arr[ $ar_k ]->share = generateShareLinks( $articles_arr[ $ar_k ]->share );
		}
	} else if ( $page > 1 || $page < 1 ) {
    header('Location: https://' . $_SERVER['SERVER_NAME'] . '/news-page.php?page=' . 1);
    exit;
  }
}

$pagePrev_link .= '/news-page.php?page=' . $pagePrev;
$pagePrev_link .= ! empty( $date ) ? '&date=' . $date : '';

$nextPage_link .= '/news-page.php?page=' . $nextPage;
$nextPage_link .= ! empty( $date ) ? '&date=' . $date : '';

function getPost( $article ) {
	$pattern = '/^([ |\t]*?)([^ |\t].*?)([ |\t]*?)$/m';
	$article->content = preg_replace($pattern, "<p>$2</p>", $article->content);
	$date = new DateTime($article->date);
	$article->date = $date->format('F d, Y');
...
<head>
	...
	<meta property="og:title" content="VODXS - Visual on demand experience solution" />
	<meta property="og:description" content="" />
	<meta property="og:url" content="https://vodxs.com/" />
	<meta property="og:site_name" content="VODXs" />
	<meta property="og:image" content="https://vodxs.com/img/vodxs-og.gif" />
	...
	<main class="content">
		...
		<div class="news-page">
			<div class="wrapper">
				<div class="news-btns">
					<div class="date-box">
						<a href="#" class="date-box-current js-dd-open"><span>Sort by Date</span></a>
						<div class="date-box-dropdown dropdown-block">
							<a href="/news-page.php?date=2018" class="date-box-dropdown-item">From 2018</a>
						</div>
					</div>
					<a href="<?php echo $pagePrev_link; ?>" class="news-btn prev" <?php if($pagePrev <= 0) {echo 'style="display: none;"';} ?>><span>Prev</span></a>
					<a href="<?php echo $nextPage_link; ?>" class="news-btn next"><span>Next</span></a>
				</div>
				<div class="news-page-list">
          <?php
            foreach ( $articles_arr as $a_key => $article ) :
          ?>
            <div class="news-page-list__item">
							<div class="news-page-list__item-image" style="background-image: url('<?php echo $article->image; ?>');">
								<a href="<?php echo $article->url; ?>"></a>
							</div>
              <div class="news-page-list__item-inf">
                <div class="news-page-list__item-name">
                  ...
                  <div class="shr-box">
                    ...
                    <div class="shr-box-dropdown dropdown-block">
                      <?php echo $article->share; ?>
                    </div>
                  </div>
								</div>
								<?php
									$date = preg_split("/[\s,]+/", $article->date);
									if ( $date[0] != undefined ) {
										$date = $date[1] . ' ' . $date[0] . ' <span>' . $date[2] . '</span>';
									}
								?>
                <div class="news-page-list__item-by">By <?php echo $article->author . ' - ' . $date; ?></div>
                <p class="news-page-list__item-text">
                  <?php echo $article->description; ?>
                  <a href="<?php echo $article->url; ?>">MORE</a>
                </p>
              </div>
            </div>
          <?php
